...
public class CommentApiController {
    ...
    public ResponseEntity<CommentDto> update(@PathVariable("articleId") Long articleId, @RequestBody CommentDto dto) {
        CommentDto commentDto = commentService.update(articleId, dto);
        return ResponseEntity.status(HttpStatus.OK).body(commentDto);
    }

    //4. 댓글 삭제
    @DeleteMapping("api/articles/comments/{articleId}")
    public ResponseEntity<CommentDto> delete(@PathVariable Long articleId) {
        CommentDto removed = commentService.remove(articleId);
